...
class Storify:
	...
	def tick(self, force=False):
		...
		while i < a:
            ...
            # We could do for i in self.databases.copy() to not get any RuntimeErrors, but it is more
			...
			if force:
				...
				if time.time() - db.lastFlush > 60 * self.save_time: # Saves every self.save_time minutes
					db.flush()
	...
	def __init__(self, name, root, log):
		...
		if os.path.exists(self.path):
			try:
				...
				self.log.fatal("Database '%s' corrupted, reading from backup..." % self.name)
	...
	def flush(self):
		...
		if os.path.exists(self.path):
			...
			try:
				...
				self.log.fatal("Possibly out of space, aggressively deleting overly-redundant backups...")
