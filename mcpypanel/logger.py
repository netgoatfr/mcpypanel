...
TF_DAYS = "%b %d %Y %H:%M:%S"
class Logger:
    def __init__(self,name,time_format = TF_HOURS,colored_output = True):
        self.master = name
        self.lock = lock
        self.time_format = time_format
        self.colored_output = colored_output
    def get_instance(self,child):
        return Logger(self.master+"#"+child,self.time_format,self.colored_output)
    ...
    def info(self,txt):
        ...
        if self.colored_output:
            print(Fore.LIGHTGREEN_EX+Style.BRIGHT+"["+self._time+"]"+" ["+self.master+"] [INFO]: "+txt+Style.RESET_ALL)
        else:
            print("["+self._time+"]"+" ["+self.master+"] [INFO]: "+txt)
        self.lock.release()
    def debug(self,txt):
        self.lock.acquire()
    ...
    def warning(self,txt):
        self.lock.acquire()
        if self.colored_output:print(Fore.YELLOW+Style.BRIGHT+"["+self._time+"]"+" ["+self.master+"] [WARN]: "+txt+Style.RESET_ALL)
        else:print("["+self._time+"]"+" ["+self.master+"] [WARN]: "+txt)

        self.lock.release()
    def error(self,txt):
        self.lock.acquire()
        if self.colored_output:print(Fore.LIGHTRED_EX+Style.BRIGHT+"["+self._time+"]"+" ["+self.master+"] [ERROR]: "+txt+Style.RESET_ALL)
        else:print("["+self._time+"]"+" ["+self.master+"] [ERROR]: "+txt)

        self.lock.release()
    def fatal(self,txt):
        self.lock.acquire()
        if self.colored_output:print(Fore.RED+Style.BRIGHT+"["+self._time+"]"+" ["+self.master+"] [FATAL]: "+txt+Style.RESET_ALL)
        else:print("["+self._time+"]"+" ["+self.master+"] [FATAL]: "+txt)
...
if __name__ == '__main__':
    ...
    log4.critical("Critical message")
